...
import { PropsForTransport, ClientMessageKind, ClientMessage } from '../types/messages';
...
export class DefaultRemoteRenderServer implements RemoteRenderServer {
  ...
  private onClientMessage = (msg: ClientMessage) => {
    switch (msg.kind) {
      ...
      case ClientMessageKind.Update: {
        const parsedProps = this.processProps(msg.id, msg.props);
        this.tellHandlers(h =>
          h.onUpdateComponent(msg.id, parsedProps)
        )
        break;
      }
      case ClientMessageKind.Unmount: {
        this.tellHandlers(h =>
          h.onUnmountComponent(msg.id)
        )
        break;
      }
      default: {
  ...
  private processProps(id: number, props: PropsForTransport): Props {
    const functionProxy = functionKey => (...params) => this.transport.sendServerMessage({ id, functionKey, params });
    const functionProps = fromPairs(props.functionProps.map(key =>
      [key, functionProxy(key)] as [string, Function]
    ));
